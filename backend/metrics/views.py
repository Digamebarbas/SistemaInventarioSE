...
class DashboardMetrics(APIView):
	...
	def get(self, request):
		...
		movements_today = InventoryMovement.objects.filter(created_at__date=today).count()

		top_products = (
			InventoryMovement.objects.filter(movement_type=InventoryMovement.TYPE_OUT)
		...
		monthly_movements = (
			InventoryMovement.objects.values("movement_type", "created_at__month")
